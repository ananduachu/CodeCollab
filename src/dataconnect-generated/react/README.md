...
## GetWorkspace
You can execute the `GetWorkspace` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetWorkspace(dc: DataConnect, options?: useDataConnectQueryOptions<GetWorkspaceData>): UseDataConnectQueryResult<GetWorkspaceData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetWorkspace(options?: useDataConnectQueryOptions<GetWorkspaceData>): UseDataConnectQueryResult<GetWorkspaceData, undefined>;
```

### Variables
The `GetWorkspace` Query has no variables.
### Return Type
Recall that calling the `GetWorkspace` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetWorkspace` Query is of type `GetWorkspaceData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetWorkspace`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useGetWorkspace } from '@dataconnect/generated/react'

export default function GetWorkspaceComponent() {
  ...
  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetWorkspace(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetWorkspace(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetWorkspace(dataConnect, options);
...
## ListFiles
You can execute the `ListFiles` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useListFiles(dc: DataConnect, vars: ListFilesVariables, options?: useDataConnectQueryOptions<ListFilesData>): UseDataConnectQueryResult<ListFilesData, ListFilesVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListFiles(vars: ListFilesVariables, options?: useDataConnectQueryOptions<ListFilesData>): UseDataConnectQueryResult<ListFilesData, ListFilesVariables>;
```
...
```
### Return Type
Recall that calling the `ListFiles` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListFiles` Query is of type `ListFilesData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListFiles`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListFilesVariables } from '@dataconnect/generated';
import { useListFiles } from '@dataconnect/generated/react'

export default function ListFilesComponent() {
  // The `useListFiles` Query hook requires an argument of type `ListFilesVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListFiles(listFilesVars);
  // Variables can be defined inline as well.
  const query = useListFiles({ workspaceId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListFiles(dataConnect, listFilesVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListFiles(listFilesVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListFiles(dataConnect, listFilesVars, options);
...
export default function CreateFileComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ workspaceId: ..., name: ..., path: ..., content: ..., language: ..., });
